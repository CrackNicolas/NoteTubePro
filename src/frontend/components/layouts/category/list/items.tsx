        ...
        <article className="relative pb-10 sm:pb-7 grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-3 w-full">
            {
                (categorys.length === 0) ?
                    ...
                    categorys.map((category: PropsCategory) => {
                        return (
                            <div key={category.title} title={`Categoria ${category.title}`} onClick={() => select(category)} className={`group col-span-1 grid place-items-center h-[100px] rounded-md cursor-pointer dark:hover:bg-dark-secondary hover:bg-custom-gradient hover:border-transparent transition duration-500 ${paint ? 'dark:hover:bg-dark-secondary hover:bg-custom-gradient dark:bg-dark-primary bg-primary dark:border-dark-secondary border-secondary border-[0.1px]' : `${category.use ? 'dark:bg-dark-secondary bg-custom-gradient' : 'dark:bg-dark-primary bg-primary dark:border-dark-secondary border-secondary border-[0.1px]'}`} `}>
                                <div className="flex flex-col items-center gap-y-1">
                                    <ComponentIcon name={category.icon} size={27} descriptionClass={`dark:group-hover:text-tertiary group-hover:text-primary ${paint ? 'dark:hover:text-seventh text-seventh' : `${category.use ? 'dark:text-dark-primary text-primary' : 'dark:text-seventh text-secondary'} `} duration-500 group-hover:translate-y-[-5px] cursor-pointer `} />
